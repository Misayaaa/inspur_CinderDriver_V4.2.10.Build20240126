...
CONF = cfg.CONF
CONF.register_opts(base.instorage_mcs_iscsi_opts)
...
class InStorageMCSISCSIDriver(instorage_common.InStorageMCSCommonDriver,
            ...
            try:
                ...
                if host_name is None:
                    with excutils.save_and_reraise_exception():
                        msg = (_('Failed to create host with connector %s') %
                                 connector)
                        LOG.error(msg)
    ...
    def _get_single_iscsi_data(self, volume, connector, iogrp_lun_id_map, chap_secret):
        LOG.debug('enter: _get_single_iscsi_data: volume %(vol)s with '
                  'connector %(conn)s iogrp_lun_id_map %(iogrp_lun_id_map)s',
                  {'vol': volume['id'], 'conn': connector,
        ...
        LOG.debug('leave: _get_single_iscsi_data:\n volume: %(vol)s\n '
                  'connector: %(conn)s\n lun_id: %(lun_id)s\n '
                  'properties: %(prop)s',
                  {'vol': volume['id'], 'conn': connector, 'lun_id': properties['target_lun'],
                   'prop': properties})
        return properties

    def _get_multi_iscsi_data(self, volume, connector, iogrp_lun_id_map, properties):
        ...
        specified_iscsi_ip_list = self.configuration.instorage_mcs_iscsi_ips
        try:
        ...
        io_groups = list(set(list(iogrp_lun_id_map.keys())))
        ...
        for node in self._state['storage_nodes'].values():
            if node['IO_group'] not in io_groups:
                continue
            for ip_data in resp:
                node_id = ip_data.get('node_id',None) or ip_data.get('nodeId',None)
                if node_id != node['id']:
                    continue
                ...
                if ((ip_data['state'] == 'configured' and
                    ...
                    if (specified_iscsi_ip_list and
                        (vaild_port_ipv4 not in specified_iscsi_ip_list and
                        vaild_port_ipv6 not in specified_iscsi_ip_list)):
                        continue
                    if vaild_port_ipv4:
                        valid_port = vaild_port_ipv4
                ...
                if valid_port and valid_port not in filter_ips:
                    ...
                    properties['target_iqns'].append(
                        node['iscsi_name'])
                    iogrp_id = node['IO_group']
                    properties['target_luns'].append(iogrp_lun_id_map[iogrp_id])

        if not properties['target_portals']:
        ...
        LOG.debug('leave: _get_multi_iscsi_data:\n volume: %(vol)s\n '
                  'connector: %(conn)s\n lun_ids: %(lun_ids)s\n '
                  'properties: %(prop)s',
                  {'vol': volume['id'], 'conn': connector, 'lun_ids': properties['target_luns'],
                   'prop': properties})
    ...
    def _do_terminate_connection(self, volume, connector, **kwargs):
        ...
        if host_name:
            ...
            #if host type is inband_management, can not delete the host.
            if (not resp
                and self.configuration.instorage_mcs_auto_delete_host
                and host_type != 'inband_management'):
                try:
